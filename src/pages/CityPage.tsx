 ...
 * two rows at 375px and stay fully visible; re-grouping later is an edit
...
const TAB_GROUPS: {
  ...
  { key: "shop", label: "שווקים", icon: "storefront", categories: ["market"] },
...
export default function CityPage() {
  ...
  const sections = TAB_GROUPS.map((tab) => {
    ...
    // reassuring empty state, rather than vanishing.
    ...
    return (
      <div className="min-h-screen bg-background px-4 py-6">
        <Link to="/" className="text-on-surface-variant">
          <Icon name="arrow_forward" className="align-middle" /> חזרה למסלול
        </Link>
      ...
      <header className="header-elevated sticky top-0 z-10 bg-primary px-4 pb-3 pt-6 text-on-primary">
        <div className="flex items-center justify-between gap-3">
          ...
          {/* Bottom NavBar is hidden on city pages, so this is the only way
              back to the dictionary from here - small and muted, on purpose,
              so it doesn't compete with the city name or the tabs below. */}
          <Link
            to="/dictionary"
